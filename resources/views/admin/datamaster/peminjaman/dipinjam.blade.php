        ...
        <div class="row">
          <div class="col-12">
            ...
            {{-- <div class="col-13" style="margin-left: 25px;">
                <a class="btn btn-primary" href="{{ route('usersCreate') }}">Tambah Data Petugas</a>
            </div> --}}
                  ...
                  <div id="table-1_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer"><div class="row"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="table-1_length"><label>Show <select name="table-1_length" aria-controls="table-1" class="form-control form-control-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-12 col-md-6"><div id="table-1_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="table-1"></label></div></div></div><div class="row"><div class="col-sm-12"><table class="table table-striped dataTable no-footer" id="table-1" role="grid" aria-describedby="table-1_info">
                    <thead>
                      <tr role="row"><th class="text-center sorting_asc" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="
                        ...
                        <th class="sorting" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-label="Task Name: activate to sort column ascending" style="width: 123.677px;">Peminjam</th>
                        {{-- <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Progress" style="width: 65.0625px;">Judul Buku</th> --}}
                        {{-- <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Members" style="width: 90.312px;">Password</th> --}}
                        <th class="sorting" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-label="Due Date: activate to sort column ascending" style="width: 110.6667px;">Tgl. Pinjam</th>
                        <th class="sorting" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-label="Due Date: activate to sort column ascending" style="width: 66.6667px;">Tgl. Pengembalian</th>
                        {{-- <th class="sorting" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-label="Due Date: activate to sort column ascending" style="width: 66.6667px;">Tgl. Kembali</th> --}}
                        <th class="sorting" tabindex="0" aria-controls="table-1" rowspan="1" colspan="1" aria-label="Status: activate to sort column ascending" style="width: 88.2917px;">Status</th>
                    ...
                    @foreach ($dipinjam as $key => $dp)
                    <tr role="row" class="odd">
                        <td class="sorting_1 align-middle">
                            {{ $dp->id_peminjaman }}
                        </td>
                        <td class="align-middle">{{ $dp->users->nama_lengkap }}</td>
                        <td class="align-middle">
                            {{ $dp->tanggal_peminjaman }}
                        </td>
                        <td class="align-middle">{{ $dp->tanggal_pengembalian }}</td>

                        <td class="align-middle" class="btn btn-info">
                            <form action="{{ route('ubah_status', $dp->id_peminjaman) }}" method="POST">
                                @csrf
                                <button type="submit" name="status" value="selesai"
                                    onclick="return confirm('apakah anda yakin ingin menyelesaikan transaksi ini?')"
                                    class="btn btn-info">{{ $dp->status_peminjaman }}</button>
                            </form>
                        </td>


                        {{-- <td class="align-middle"><span class="btn btn-info">{{ $dp->status_peminjaman }}</span></td> --}}
                         <td class="d-flex">
                            <a href="{{ route('showPeminjaman', $dp->id_peminjaman) }}" style="margin-right: 2px;" class="btn btn-primary"><i class="menu-icon fa fa-eye"></i></a>
                                {{--< a href="{{ route('usersEdit', $us->id) }}" style="margin-right: 2px;" class="btn btn-warning"><i class="menu-icon fa fa-edit"></i></a>--}}
                                {{-- <a href="{{ route('peminjamDelete', $us->id) }}" style="margin-right: 2px;" class="btn btn-danger" onclick="return confirm('Apakah anda ingin menghapus data')"><i class="fa fa-trash"></i></a> --}}
                        </td>
